...
def main(settings):
    ...
    while True:
		...
		# Update all servos:
		for actuator in myActuators:
			if ( actuator.id == di.LHR):
				actuator.goal_position = rad2dyn(dyno.LHR)
        
			if ( actuator.id == di.LHP):
				actuator.goal_position = rad2dyn(dyno.LHP)
				
			if ( actuator.id == di.LKP):
				actuator.goal_position = rad2dyn(dyno.LKP)
        
			if ( actuator.id == di.LAP):
				actuator.goal_position = rad2dyn(dyno.LAP)
			
			
			if ( actuator.id == di.CHR):
				actuator.goal_position = rad2dyn(dyno.CHR)
        
			if ( actuator.id == di.CHP):
				actuator.goal_position = rad2dyn(dyno.CHP)
				
			if ( actuator.id == di.CKP):
				actuator.goal_position = rad2dyn(dyno.CKP)
        
			if ( actuator.id == di.CAP):
				actuator.goal_position = rad2dyn(dyno.CAP)
			
			
			if ( actuator.id == di.RHR):
				actuator.goal_position = rad2dyn(dyno.RHR)
        
			if ( actuator.id == di.RHP):
				actuator.goal_position = rad2dyn(dyno.RHP)
				
			if ( actuator.id == di.RKP):
				actuator.goal_position = rad2dyn(dyno.RKP)
        
			if ( actuator.id == di.RAP):
				actuator.goal_position = rad2dyn(dyno.RAP)
